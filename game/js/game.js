...
function equipItem(itemType) {
  let elementId =
    itemType == "weapon" ? "character-weapons" : "character-shields";
...
function showEquippedItems() {
  ...
  if (
    "id" in character.equippedWeapon &&
    !weaponIds.includes(character.equippedWeapon.id)
  ) {
  ...
  if (
    "id" in character.equippedShield &&
    !weaponIds.includes(character.equippedShield.id)
  ) {
...
function heal() {
  ...
  if (
    confirm(
      `Are you sure you want to heal? It will cost you ${healPrice} gold coins.`
    )
  ) {
